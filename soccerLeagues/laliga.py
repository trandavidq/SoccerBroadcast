...
url_epl = 'https://www.theguardian.com/football/laligafootball/table'
...
def delete_db(tableName):
    if(path.isfile(tableName)):
        print('La liga updating db')
        os.remove(tableName)
    else:
        print('La Liga nothing to delete')

delete_db('laliga_table.db')
create_and_initialize_db('laliga_table.db',teams_epl_dict)