...
url_bundesliga = 'https://www.theguardian.com/football/ligue1football/table'
...
def create_and_initialize_db(tableName,teams_epl_dict):
    print('ligue 1 creating')
    connection = sqlite3.connect(tableName)
    cursor = connection.cursor()

    # table already created
    cursor.execute("""CREATE TABLE IF NOT EXISTS ligue1_table (
                position INTEGER,
    ...
    for t in teams_epl_dict.keys():
        cursor.execute("INSERT OR IGNORE INTO ligue1_table (position, team_name, games_played, wins, draws, losses, goal_differential, points) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
...
def delete_db(tableName):
    if(path.isfile(tableName)):
        print('ligue 1 updating db')
        os.remove(tableName)
    else:
        print('ligue 1 nothing to delete')

delete_db('ligue1_table.db')
create_and_initialize_db('ligue1_table.db',teams_bundesliga_dict)