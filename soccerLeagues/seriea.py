...
url_bundesliga = 'https://www.theguardian.com/football/serieafootball/table'
...
def create_and_initialize_db(tableName,teams_epl_dict):
    print('serie a creating')
    connection = sqlite3.connect(tableName)
    cursor = connection.cursor()

    # table already created
    cursor.execute("""CREATE TABLE IF NOT EXISTS seriea_table (
                position INTEGER,
    ...
    for t in teams_epl_dict.keys():
        cursor.execute("INSERT OR IGNORE INTO seriea_table (position, team_name, games_played, wins, draws, losses, goal_differential, points) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
...
def delete_db(tableName):
    if(path.isfile(tableName)):
        print('serie a updating db')
        os.remove(tableName)
    else:
        print('serie a nothing to delete')

delete_db('seriea_table.db')
create_and_initialize_db('seriea_table.db',teams_bundesliga_dict)